...
int main() {
   ...
   //pular duas linhas para melhor visualizacao
    ...
    if(densidade1 > densidade2) {
        printf("Carta 1 - %s (%s): %.2f\n", estado1, codigo_carta1, densidade1);
        printf("Carta 2 - %s (%s): %.2f\n", estado2, codigo_carta2, densidade2);
        printf("Resultado: Carta 1 (%s) venceu em densidade demografica!\n", estado1);
    } 
    else if (densidade2 > densidade1){
        printf("Carta 1 - %s (%s): %.2f\n", estado1, codigo_carta1, densidade1);
        printf("Carta 2 - %s (%s): %.2f\n", estado2, codigo_carta2, densidade2);
        printf("Resultado: Carta 2 (%s) venceu em densidade demografica!\n", estado2);
    } 
    else {
        printf("As duas cidades possuem a mesma densidade demografica. Empate!\n");
    }
    if (pibper_capita1 > pibper_capita2) {
        printf("Carta 1 - %s (%s): %.2f\n", estado1, codigo_carta1, pibper_capita1);
        printf("Carta 2 - %s (%s): %.2f\n", estado2, codigo_carta2, pibper_capita2);
        printf("Resultado: Carta 1 (%s) venceu em pib per capital!\n", estado1);
    } 
    else if (pibper_capita2 > pibper_capita1){
        printf("Carta 1 - %s (%s): %.2f\n", estado1, codigo_carta1, pibper_capita1);
        printf("Carta 2 - %s (%s): %.2f\n", estado2, codigo_carta2, pibper_capita2);
        printf("Resultado: Carta 2 (%s) venceu em pib per capital!\n", estado2);
    } 
    else {
        printf("As duas cidades possuem o mesmo pib per capital. Empate!\n");
    }
